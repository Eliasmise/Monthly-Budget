...
do $$
declare
  seed_admin text := coalesce(
    nullif(current_setting('app.settings.seed_admin_username', true), ''),
    nullif(current_setting('SEED_ADMIN_USERNAME', true), ''),
    'admin'
  );
begin
  insert into public.users (username, role)
  values (seed_admin, 'admin')
